...
linear2 <- function(y,X){
  n <- nrow(X)
  ii <- sample(1:n) #shuffle the indices
  xtest <- X[ii[1:(n/5)],]; ytest <- y[ii[1:(n/5)]]
  xtrain <- X[-ii[1:(n/5)],]; ytrain <- y[-ii[1:(n/5)]]
  beta <- solve(t(xtrain)%*%xtrain,t(xtrain)%*%ytrain)
  preds <- xtest%*%beta
  mspe <- mean((ytest-preds)^2)
  return(list(beta=beta,mspe=mspe))
}
